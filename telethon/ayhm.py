...
ZelzalSelf_cmd = (
    ...
    "\n 𓆩 [Mixthon](t.me/mixthon) 𓆪"
)

@Ze.on(pattern="الذاتيه")
async def cmd(zelzallll):
    await edit_or_reply(zelzallll, ZelzalSelf_cmd)

@Ze.on(pattern=f"{POSC}(?: |$)(.*)")
async def oho(event):
...
@Ze.on(pattern="(تفعيل الذاتيه|تفعيل الذاتية)")
...
@Ze.on(pattern="(تعطيل الذاتيه|تعطيل الذاتية)")
